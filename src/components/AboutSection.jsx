...
export const AboutSection = () => {
  ...
  const handleOpenMessage = (leaderData, event) => {
    triggerElementRef.current = event?.currentTarget || null;
  ...
  return (
    <section id="about" className="py-20 bg-white">
            ...
            {FOUNDERS_MEMORIAL.map((founder, idx) => (
              ...
              >
                ...
                <div className="flex-1 flex flex-col justify-between text-center sm:text-left h-full">
                  ...
                  <div>
                    {/* Reverent Quote */}
                    <div className="p-3.5 rounded-2xl bg-white border border-blue-100 text-xs italic text-slate-700 shadow-xs mb-3">
                      <Quote className="w-4 h-4 text-blue-400 mb-1 inline mr-1" />
                      <span>{founder.quote}</span>
                    </div>

                    <div className="pt-3 border-t border-blue-200/70 flex items-center justify-between">
                      ...
                      <button
                        ...
                        aria-label={`Read Message from ${founder.name}`}
                      >
              ...
              {upperLeadership.map((leader) => (
                ...
                >
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <button
                      onClick={(e) => handleOpenMessage(leader, e)}
                      className="text-xs font-bold text-blue-700 hover:text-blue-900 flex items-center gap-1 cursor-pointer transition-colors"
      ...
      {/* Leadership & Founders Full Message Modal */}
      <AnimatePresence>
        {selectedLeader && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-leader-title"
          >
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border-2 border-blue-100 max-h-[90vh] overflow-y-auto flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header */}
              <div className="flex items-start justify-between pb-5 border-b border-blue-100">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-20 sm:w-18 sm:h-24 rounded-2xl overflow-hidden ring-4 ring-blue-100/80 shadow-md shrink-0 bg-slate-200">
                    <ImageWithFallback
                  ...
                  <div>
                    <span className="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-blue-100 text-blue-800 mb-1">
                      {selectedLeader.designation}
                    </span>
                    <h3 id="modal-leader-title" className="text-xl sm:text-2xl font-bold text-slate-900 font-serif">
                      {selectedLeader.name}
                    </h3>
                    {selectedLeader.qualifications && (
                      <div className="text-xs text-blue-700 font-semibold mt-0.5">
                        Qualifications: {selectedLeader.qualifications}
                      </div>
                    )}
                    {selectedLeader.motto && (
                      <div className="text-xs text-slate-500 italic mt-0.5">
                        {selectedLeader.motto.startsWith('"') ? selectedLeader.motto : `"${selectedLeader.motto}"`}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center font-bold transition-colors cursor-pointer shrink-0 focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              {/* Message Content */}
              <div className="py-6 space-y-4 text-slate-700 leading-relaxed text-sm sm:text-base">
                {selectedLeader.fullMessage ? (
                  selectedLeader.fullMessage.split('\n\n').map((paragraph, pIdx) => {
                  ...
                  <p className="whitespace-pre-line">{selectedLeader.excerpt}</p>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-blue-100 flex items-center justify-between">
                ...
                <button
                  onClick={() => setSelectedLeader(null)}
                  className="px-5 py-2.5 rounded-xl bg-blue-700 text-white text-xs sm:text-sm font-bold hover:bg-blue-800 transition-colors cursor-pointer shadow-xs focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
                  Close Message
                </button>
              </div>
